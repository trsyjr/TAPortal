...
import Advisory from "./Advisory"; // Imported to capture tracking intent before form entry
...
const TicketModal = ({ isOpen, onClose }) => {
  ...
  // Track whether the user has explicitly bypassed the security advisory for this individual modal open action
  ...
  useEffect(() => {
    if (!isOpen) {
      ...
      setIsCategoryOpen(false);
      setIsCommunicationOpen(false);
      setHasPassedAdvisory(false); // Reset gatekeeper tracking when modal closes
    }
  }, [isOpen, reset]);
    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          {/* Gatekeeper: If user hasn't passed the advisory confirmation yet, force show it */}
          {!hasPassedAdvisory ? (
            <Advisory 
              forceShow={true} 
            ...
            /* Ticket Form Content Container - Reveals once advisory passes */
            <motion.div
